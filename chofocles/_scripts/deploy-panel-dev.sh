...
# deploy-panel-dev.sh — Despliega chofocles/panel a saycudev.
#
# Pasos:
#   1) Vendor-copy de saycu-theme/ adyacente al panel (lo que requiere el
#      Dockerfile para que `file:../saycu-theme` resuelva).
#   2) rsync del proyecto chofocles/ a saycudev.
#   3) docker compose build chofocles_panel + up -d.
...
log "2) rsync chofocles → $REMOTE_HOST:$REMOTE_DIR/"
rsync -az --delete \
...
log "4) docker compose build + up -d (chofocles_panel)"
ssh "$REMOTE_HOST" "cd $REMOTE_DIR && docker compose build panel && docker compose up -d panel" 2>&1 | tail -20

log "4) Verificación"
sleep 2
ssh "$REMOTE_HOST" "docker ps --filter name=chofocles_panel --format 'table {{.Names}}\t{{.Status}}'"

log "OK. Probar:  https://dev-panel.superapi.eoden.es/chofocles/"
